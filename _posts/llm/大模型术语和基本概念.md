...
稠密（Dense）模型指模型中的所有参数在计算时会被激活和使用。Transformer 模型大多是稠密的。优点是模型容量大，表达能力强。缺点是计算和存储的开销高。
稀疏（Sparse）模型指模型中只有部分参数被激活和使用。典型的如 MoE 模型就是稀疏模型。优点是计算效率高，计算成本不随模型规模线形增加。缺点是实现复杂。