...
author: bsyonline
lede: 没有摘要
date: 2021-09-17 08:59:23
thumbnail:
---



## Part. I Cache Fundamentals
...
能使用缓存的场景很多，需要结合实际业务选择合适方案。

### 缓存数据库双写一致性问题

缓存和数据库双写有 3 种比较成熟的模式：

**Cache Aside Pattern**

读的时候先读缓存，如果没有命中，那就读数据库，然后取出数据放入缓存并返回响应。

更新的时候，先删除缓存，在更新数据库。

优点简单，缺点同时操作缓存和数据库，可能会有数据不一致。
...
通常用的最多的就是 Cache Aside Pattern 。但是在读写高并发场景会有缓存数据库不一致的情况。

**更新缓存 or 删除缓存**

1. 更新缓存会导致高并发场景缓存和数据库不一致。
...
2. 缓存重建成本
   ...
   如果比较复杂，在频繁更新场景，会做无用功，不如查询时重建效果好。

**缓存数据库不一致出现的场景**

不管是先删缓存在更新数据库还是先更新数据库再删除缓存，都有可能出现不一致。

1. 先删除缓存再更新数据库

   ```
step1：写请求A先删除缓存。
   step2：读请求B先去查缓存，没有命中，就去查数据库。
step3：读请求B将旧数据写到缓存。
   step3：写请求A更新数据库成功。
```
...
2. 先更新数据库再删除缓存
   ...
   ```
step1：读请求A先查缓存，没有命中，再查数据库。
   step2：写请求B更新数据库成功。
step3：写请求B删除缓存。
   step4：读请求A重建缓存，此时缓存是旧数据。
```
   
大多是情况下，读请求会比写请求处理更快，所以出现此种情况的可能性非常低，但不能保证一定没有。

**读写高并发场景缓存数据库不一致解决方案**

在内存中维护一组请求处理队列，先来了一个写请求，先将写请求根据 hash 放到一个内存队列中，在有一个线程从内存队列中取出请求进行处理。还是先删除缓存，在更新数据库。

此时来了一个读请求，先去查缓存，没有命中，再看下队列中是否有操作相同数据的写请求，如果有就进队列排队。如果没有就查数据库，然后写缓存。

读请求出队列先查缓存，再查数据库，如果命中就直接返回，如果没命中，再查数据库，再写缓存。这样如果有很多个读请求排队，只有第一个读请求会查一个数据库，后续的操作就缓存，不用再查数据库。

此方案需要主要的点：

1. 需要将读写请求都路由到同一个机器的内存队列，可以通过 nginx hash 路由来做。
2. 如果写请求阻塞的读请求较多的情况下，可以考虑当队列中有读请求排队新的读请求就不进队列排队，再队列外面阻塞等待（需要根据业务评估超时时间）。
